<?php include("header.php"); ?> 
    <div class="container wow fadeInUp" style="padding-left: 10%; padding-right: 10%;">
        <div class="row" style="padding-top: 50px;">
            <div class="col-md-12" id="about" style="line-height: 3rem;">
                <div class="col-md-4 visible-lg visible-md visible-sm">
                    <img src="assets/logo/big%20logo.png" class="img-responsive">
                </div>
                <div class="col-md-8">
                    <h1 id="header__black" style="line-height: 25px; padding-top: 50px;">about us<br><span style="color: #657ab3;">Automac Multiresources</span></h1>
                    <p style="text-align: justify;"><b>Automac Multiresources Sdn. Bhd. (AMSB)</b> is a Bumiputra Company, acquired in 1983 with a humble beginning of providing blasting &amp; painting services in the oil &amp; gas sector, particularly in BLNG. AMSB is a member of Adinin Group of Companies
                    </p>
                    <hr>
                    <div class="row">
                        <div class="col-md-12" id="contact" style="line-height: 3rem;">
                            <h1 id="header__black" style="line-height: 25px;">Contact us</h1>
                            <dl class="row" style="line-height: 20px;">
                              <dt class="col-sm-2">HEAD OFFICE</dt>
                              <p class="col-sm-10">Lot 4157, Simpang 85, Jalan Maulana, Kampong Pandan A<br>Kuala Belait<br>KA1931<br>Negara Brunei Darussalam
                              </p>
                              <br>    
                              <dt class="col-sm-2">ESS OFFICE</dt>
                              <p class="col-sm-10">No. 4, Spg. 236, Kg. Katok Tungku,<br>Gadong, BE2319<br>Bandar Seri Begawan<br>Negara Brunei Darussalam
                              </p>
                              <br>  
                              <dt class="col-sm-2">POSTAL ADDRESS</dt>
                              <p class="col-sm-10">PO Box 78<br>Kuala Belait KA1531<br>Negara Brunei Darussalam
                              </p>
                              <br>
                              <dt class="col-sm-2">Tel</dt>
                              <p class="col-sm-10">+673 - 3348132(KB) / +673 - 2232265(BSB)</p>
                              <br> 
                              <dt class="col-sm-2">Fax</dt>
                              <p class="col-sm-10">+673 - 3347190(KB) / +673 - 2232265(BSB)</p>
                              <br> 
                              <dt class="col-sm-2">Email</dt>
                              <p class="col-sm-10">+673 - 3347190(KB) / +673 - 2232265(BSB)</p>
                            </dl>  
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <hr>
<!--
        <div class="row">
            <div class="col-md-12" id="contact" style="line-height: 3rem;">
                <h1 id="header__black" style="line-height: 25px;">Contact us</h1>
                <dl class="row" style="line-height: 20px;">
                  <dt class="col-sm-2">HEAD OFFICE</dt>
                  <p class="col-sm-10">Lot 4157, Simpang 85, Jalan Maulana, Kampong Pandan A<br>Kuala Belait<br>KA1931<br>Negara Brunei Darussalam
                  </p>
                  <br>    
                  <dt class="col-sm-2">ESS OFFICE</dt>
                  <p class="col-sm-10">No. 4, Spg. 236, Kg. Katok Tungku,<br>Gadong, BE2319<br>Bandar Seri Begawan<br>Negara Brunei Darussalam
                  </p>
                  <br>  
                  <dt class="col-sm-2">POSTAL ADDRESS</dt>
                  <p class="col-sm-10">PO Box 78<br>Kuala Belait KA1531<br>Negara Brunei Darussalam
                  </p>
                  <br>
                  <dt class="col-sm-2">Tel</dt>
                  <p class="col-sm-10">+673 - 3348132(KB) / +673 - 2232265(BSB)</p>
                  <br> 
                  <dt class="col-sm-2">Fax</dt>
                  <p class="col-sm-10">+673 - 3347190(KB) / +673 - 2232265(BSB)</p>
                  <br> 
                  <dt class="col-sm-2">Email</dt>
                  <p class="col-sm-10">+673 - 3347190(KB) / +673 - 2232265(BSB)</p>
                </dl>  
            </div>
        </div>
-->
      </div>
<?php include("footer.php"); ?>